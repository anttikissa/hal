...
function makeSleeperToolStream(command: string): ReadableStream<Uint8Array> {
	const enc = new TextEncoder()
	const sse = (event: string, data: any) => enc.encode(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`)
		...
		async pull(c) {
			switch (step++) {
				case 0:
					c.enqueue(sse('message_start', { type: 'message_start', message: { usage: { input_tokens: 10, output_tokens: 0 } } }))
					c.enqueue(sse('content_block_start', { type: 'content_block_start', index: 0, content_block: { type: 'text' } }))
					c.enqueue(sse('content_block_delta', { type: 'content_block_delta', index: 0, delta: { type: 'text_delta', text: 'Running long command.\n' } }))
					c.enqueue(sse('content_block_stop', { type: 'content_block_stop', index: 0 }))
					break
				case 1:
					c.enqueue(sse('content_block_start', { type: 'content_block_start', index: 1, content_block: { type: 'tool_use', id: 'mock_sleep_1', name: 'bash' } }))
					break
				case 2:
					await new Promise(r => setTimeout(r, 50))
					c.enqueue(sse('content_block_delta', { type: 'content_block_delta', index: 1, delta: { type: 'input_json_delta', partial_json: input } }))
...
type MockResponse = { chunks: string[]; delayMs: number; error?: boolean; tool?: boolean; sleepCommand?: string }
function generateResponse(messages: any[]): MockResponse {
	const last = messages[messages.length - 1]
	const isToolResult = last?.role === 'user' && Array.isArray(last.content) && last.content.some((c: any) => c.type === 'tool_result')
...
class MockProvider extends Provider {
	...
	async fetch(body: any, _signal?: AbortSignal) {
		const { chunks, delayMs, error, tool, sleepCommand } = generateResponse(body.messages ?? [])
		if (error) {
			return new Response(JSON.stringify({ type: 'error', error: { type: 'invalid_request_error', message: 'Mock error' } }), {
				status: 400, headers: { 'Content-Type': 'application/json' },
			})
		}
		const stream = sleepCommand ? makeSleeperToolStream(sleepCommand) : tool ? makeToolStream() : makeSSEStream(chunks, delayMs)
