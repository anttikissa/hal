...
import { renderStatus } from '../src/client/terminal/render-status.ts'
import { renderHistory } from '../src/client/terminal/render-history.ts'
...
describe('render', () => {
	...
	test('paused tab alert uses the configured bright foreground', () => {
		...
		try {
			...
			expect(output).toContain(`${colors.tab.pausedFg}!`)
		} finally {
			cursor.isVisible = originalIsVisible
		}
	})
...
test('chrome fade uses every heartbeat without repainting history or changing layout', () => {
	...
	const originalBackground = keys.state.background
	...
	try {
		...
		expect(render.hasAnimatedIndicators(false, false)).toBe(false)
	} finally {
		cursor.heartbeatTick = originalTick
		keys.state.background = originalBackground
