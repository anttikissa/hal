...
function tempSession(): string {
	...
	ensureDir(dir)
	// Write minimal meta.ason with log field
...
describe('rotateLog', () => {
	test('rotates to messages2.asonl', async () => {
		...
		await appendMessages(id, [{ role: 'user', content: 'hello', ts: new Date().toISOString() } as Message])

		const newLog = await rotateLog(id)
		expect(newLog).toBe('messages2.asonl')
	...
	test('increments rotation number', async () => {
		const id = tempSession()
		await appendMessages(id, [{ role: 'user', content: 'r1', ts: new Date().toISOString() } as Message])

		expect(await rotateLog(id)).toBe('messages2.asonl')

		// Write to new log
		await appendMessages(id, [{ role: 'user', content: 'r2', ts: new Date().toISOString() } as Message])
		expect(await rotateLog(id)).toBe('messages3.asonl')

		await appendMessages(id, [{ role: 'user', content: 'r3', ts: new Date().toISOString() } as Message])
		expect(await rotateLog(id)).toBe('messages4.asonl')
	})

	test('appendMessages writes to current log after rotation', async () => {
		const id = tempSession()
		await appendMessages(id, [{ role: 'user', content: 'before', ts: new Date().toISOString() } as Message])

		await rotateLog(id)

		await appendMessages(id, [{ role: 'user', content: 'after', ts: new Date().toISOString() } as Message])

		// Old file should have 'before'
		const oldRaw = readFileSync(join(sessionDir(id), 'messages.asonl'), 'utf-8')
		expect(oldRaw).toContain('before')
		expect(oldRaw).not.toContain('after')

		// New file should have 'after'
		const newRaw = readFileSync(join(sessionDir(id), 'messages2.asonl'), 'utf-8')
