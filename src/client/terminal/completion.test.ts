...
test('/model completes current model aliases and bare model ids', () => {
	...
	const bare = completion.complete('/model gemini-3.8-f', '/model gemini-3.8-f'.length)

	expect(alias).not.toBeNull()
	expect(alias!.items).toContain('/model gemini')
	expect(alias!.items).toContain('/model gemini-3.8-flash')
	expect(bare).not.toBeNull()
	expect(bare!.items).toContain('/model gemini-3.8-flash')
})
