...
test('spawn_agent block contains wrapped input and output', () => {
	const block: Block = {
		type: 'tool',
		name: 'spawn_agent',
		toolId: 'call_123',
	...
	expect(body).toContain('Spawn agent')
...
test('send block identifies delivered and queued messages without repeating its status in the body', () => {
	...
	expect(headerLine(sentLines)).toContain('Sent message to 33-270')
	...
	expect(headerLine(queuedLines)).toContain('Queued message for 33-270')
...
test('grep block quotes its search pattern in header', () => {
	const block: Block = {
		type: 'tool',
		name: 'grep',
		input: { pattern: 'const MODEL_GROUPS', path: '/home/user/.hal/src' },
	}

	const header = headerLine(blocks.renderBlock(block, 100))
	expect(header).toContain('Grep "const MODEL_GROUPS" in /home/user/.hal/src')
})
...
test('bash block strips redundant cd prefix for the current cwd', () => {
	const block: Block = {
		type: 'tool',
		name: 'bash',
		input: { command: 'cd /home/user/.hal && git status', cwd: '/home/user/.hal/' },
	}

	const header = headerLine(blocks.renderBlock(block, 100))
	expect(header).toContain('Bash: git status')
	expect(header).not.toContain('cd /home/user/.hal')
})
...
test('bash git commit renders rich commit details instead of shell plumbing', () => {
	const block: Block = {
		type: 'tool',
		name: 'bash',
		input: { command: 'cd /home/user/.hal && git commit -m "Nice title\n\nGenerated by test"', cwd: '/home/user/.hal' },
...
[/hal-commit]`,
	...
	expect(lines.join('\n')).not.toContain('cd /home/user/.hal')
})
