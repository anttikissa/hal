...
function hint(sessionId?: string | null): string | null {
	...
	if (!active) return null
	if (active.mode === 'amend') return 'editing just-sent prompt — enter: send edited · ↓: continue original only unchanged · esc: stay paused'
	if (active.mode === 'cancel') return 'editing just-sent prompt — enter: send edited · ↓: continue original only unchanged · esc: continue original'
