...
export function parseOsc8Uri(seq: string): string | null {
	...
	let end = seq.length
	if (seq.endsWith('\x07')) end--
	else if (seq.endsWith('\x1b\\')) end -= 2
	return seq.slice(secondSemi + 1, end)
}

function countChar(s: string, ch: string, end: number): number {
	let n = 0; for (let i = 0; i < end; i++) if (s[i] === ch) n++; return n
}

const PROSE_TRIM = '.,:;!?\'">', MD_TRIM = '*`_~'
const WRAP_PAIRS = '``**__~~""\'\'<>()[]{}' // 2-char pairs
...
export function normalizeDetectedUrl(url: string): string {
	let value = url.trim(), changed = true
	while (changed && value.length > 1) {
		changed = false
		const first = value[0], last = value[value.length - 1]
		for (let p = 0; p < WRAP_PAIRS.length; p += 2) {
			if (first === WRAP_PAIRS[p] && last === WRAP_PAIRS[p + 1]) {
				value = value.slice(1, -1).trim(); changed = true; break
			}
		}
		if (changed) continue
		const trimmed = trimTrailing(value, false)
		if (trimmed.length < value.length) { value = trimmed; changed = true }
	}
	return trimTrailing(value, true)
}

function stripAnsiMap(line: string): { plain: string; posMap: number[] } {
	const chars: string[] = [], posMap: number[] = []
	let i = 0
	while (i < line.length) {
		if (line[i] === '\x1b') { i += readEscapeSequence(line, i); continue }
		posMap.push(i); chars.push(line[i]); i++
	}
...
export function urlAtCol(line: string, col: number): string | null {
	...
	for (const u of findUrlsInPlain(plain)) if (col >= u.start && col < u.end) return u.url
	return null
}

export function underlineOsc8Link(line: string, targetUrl: string): string {
	let result = '', i = 0, inTarget = false
	while (i < line.length) {
		if (line[i] === '\x1b') {
			const len = readEscapeSequence(line, i)
			...
			if (uri !== null) {
				...
				result += seq; inTarget = uri === targetUrl
