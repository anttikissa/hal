...
export const anthropicProvider: Provider = {
	...
	parseSSE(rawEvent: { type: string; data: string }): StreamEvent[] {
		...
		if (event.type === "content_block_start") {
			...
			if (block.type === "tool_use") return [{ type: "tool_use_start", index: idx, id: block.id, name: block.name }]
			...
			if (block.type === "web_search_tool_result") {
				const results = (block.content || [])
					...
					.join("\n")
				const events: StreamEvent[] = [{ type: "raw_block", index: idx, block: { ...block } }]
		...
		if (event.type === "content_block_delta") {
			...
			const idx = event.index
			if (delta.type === "thinking_delta") return [{ type: "thinking_delta", index: idx, text: delta.thinking }]
			if (delta.type === "text_delta") return [{ type: "text_delta", index: idx, text: delta.text }]
			if (delta.type === "input_json_delta") return [{ type: "tool_input_delta", index: idx, json: delta.partial_json }]
			if (delta.type === "signature_delta") return [{ type: "signature_delta", index: idx, signature: delta.signature }]
		}
		...
		if (event.type === "message_delta") {
			...
			if (event.delta?.stop_reason) events.push({ type: "stop", stopReason: event.delta.stop_reason })
	...
	addCacheBreakpoints(msgs: any[]): any[] {
		...
		if (cloned.length >= 3) {
			for (let i = cloned.length - 2; i >= 0; i--) {
				if (cloned[i].role === "user") { addBreakpoint(cloned[i]); break }
			}
		}
		return cloned
	},
