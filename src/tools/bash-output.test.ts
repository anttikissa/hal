...
test('bash turns escaped newlines in hal commit messages into real newlines before commit', async () => {
	const dir = mkdtempSync(join(tmpdir(), 'hal-bash-commit-newlines-'))
	try {
		Bun.spawnSync(['git', 'init'], { cwd: dir, stdout: 'ignore', stderr: 'ignore' })
		Bun.spawnSync(['git', 'config', 'user.email', 'a@test.com'], { cwd: dir })
		Bun.spawnSync(['git', 'config', 'user.name', 'Test'], { cwd: dir })
		...
		const out = await bash.execute(
			{ command: 'git add a.ts && git commit -m "add a" -m "Body line.\\n\\nGenerated by hal/test (session s)"' },
			{ sessionId: 's', cwd: dir },
		)
