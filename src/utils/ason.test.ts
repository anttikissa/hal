...
describe('stringify', () => {
	...
	describe('string quoting', () => {
		...
		test('double quote in string → single quotes', () => expect(stringify('she said "hi"')).toBe("'she said \"hi\"'"))
		test('both quotes → single quotes, escape single', () => expect(stringify(`she said "it's fine"`)).toBe("'she said \"it\\'s fine\"'"))
	})
	...
	describe('objects', () => {
		...
		test('object with mixed values', () => expect(stringify({ name: 'hal', version: 1 })).toBe("{ name: 'hal', version: 1 }"))
		test('wide object breaks lines', () => {
			const obj = { alpha: 'something long', beta: 'another thing', gamma: 'third value', delta: 'fourth item' }
			expect(stringify(obj)).toBe(
"{\n  alpha: 'something long',\n  beta: 'another thing',\n  gamma: 'third value',\n  delta: 'fourth item',\n}")
		})

		test('nested wide object', () => {
			const obj = { outer: { alpha: 'something', beta: 'another', gamma: 'third', delta: 'fourth' } }
			expect(stringify(obj)).toBe(
"{\n  outer: {\n    alpha: 'something',\n    beta: 'another',\n    gamma: 'third',\n    delta: 'fourth',\n  },\n}")
		})
...
"[\n  'something longer',\n  'another thing here',\n  'third value is big',\n  'fourth item too',\n]")
		...
		test('wide array of objects breaks lines', () => {
			const obj = [{ name: 'alice', score: 100 }, { name: 'bob', score: 200 }, { name: 'charlie', score: 300 }]
			expect(stringify(obj)).toBe(
...
describe('parse', () => {
	...
	describe('arrays', () => {
		...
		test('nested', () => expect(parse('[[1, 2], [3, 4]]')).toEqual([[1, 2], [3, 4]]))
	...
	describe('comments', () => {
		...
		test('comment in object', () => expect(parse('{ a: 1, // comment\n b: 2 }')).toEqual({ a: 1, b: 2 }))
...
async function collect(stream: ReadableStream<Uint8Array>, options?: Parameters<typeof parseStream>[1]): Promise<any[]> {
	const results: any[] = []
	...
	test('parseStream recover skips primitive from corrupted prefix and resumes at next object', async () => {
		const values = await collect(toStream([
