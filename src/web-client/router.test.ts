...
beforeEach(() => {
	fakeBrowser('https://example.test/')
	router.handlePopState()
})

test('parses the session id from the first path segment', () => {
	expect(router.parse('https://example.test/05-wan')).toBe('05-wan')
	expect(router.parse('https://example.test/112-bad')).toBe('112-bad')
})

test('treats the root and non-session paths as no session', () => {
	expect(router.parse('https://example.test/')).toBe('')
	expect(router.parse('https://example.test/styles.css')).toBe('')
	expect(router.parse('https://example.test/main.js')).toBe('')
	expect(router.parse('https://example.test/deep/link')).toBe('')
})
