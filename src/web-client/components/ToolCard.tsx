...
export function ToolCard(props: ToolCardProps) {
	...
	return <article class={['ToolCard', `ToolCard-${props.tool.name}`, { running: !!props.tool.running }]} aria-label={card().title}>
		<details id={props.tool.blobId ? `tool-${encodeURIComponent(props.tool.blobId)}` : undefined}>
			<summary>
			...
			<Show when={props.tool.blobId}><a class="ToolCard-link" href={router.toolHash(props.tool.blobId!)}>Link to this tool</a></Show>
