...
- Commit messages: use a separate `-m` option for the title, each body paragraph, and the `Generated by <agent>/<model> (session <session-id>)` footer. Never embed `\n` in a message argument. Copy the exact values from the opening `<agent>` and `<model>` tags; do not normalize either value.
