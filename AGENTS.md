...
- Commit messages: short title, followed by longer message if needed, then "Generated by <agent>/<model> (session <session-id>)". Copy the exact values from the opening `<agent>` and `<model>` tags; do not normalize either value.
