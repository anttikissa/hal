...
- Never signal, kill, or restart a Hal process unless the user explicitly requests it in the current turn. Before cleaning up any suspected test process, inspect the live host lock and client registry and exclude every Hal host/client PID; prefer isolated test-harness cleanup.
