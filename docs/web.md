...
- Load `colors.css` once at page load and reload it only on the server's WebSocket `colors-changed` message. **Never poll static assets or configuration from the browser**: periodic requests for unchanged data are unacceptable.
...
move between tabs. `#<blockId>` highlights any transcript block by its short
ID and expands tool cards; `#paste=<entryId>` opens a saved text paste. Those
targets can arrive after a session snapshot, so the transcript retries focus as
data loads.
